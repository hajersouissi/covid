...
class PersonFormScreen extends Component {
  static navigationOptions = ({navigation}) => {
    return {
      title: navigation.getParam ('person')
        ? i18n.t ('personForm:editContact')
        : i18n.t ('personForm:newContact'),
    };
  };
  ...
  componentDidMount () {
    ...
    if (currentPerson) {
      this.setState (prevState => ({
        person: (prevState.person = currentPerson),
      }));
    }
  }
  ...
  submitSymptoms = () => {
    ...
    if (symptom && symptom.length > 2) {
      this.setState (prevState => ({
        person: {
          ...prevState.person,
          symptoms: [...prevState.person.symptoms, symptom],
        },
      }));
    }
  };
